...
function drawRulesSection() {
    ...
    textSize(34);
    textFont('Helvetica');
    text("SPACE STRATEGO", centerX, centerY - 300);
    ...
    const rules = "The game can be used as a Friday Fun or team building activity \n" +
        "where two teams play against each other in the fight for the flag.\n" +
        "The intention is that each teams has a group call to knowledge share   \n" +
        "and discuss strategy doing the game. It also makes it much more fun.\n\n" +
        "Use the keys WASD to move around on the planet, TFGH to move around  \n" +
        "within the visible part of the screen and click with the mouse to shoot.  \n" +
        "(only available for some characters). Use Warp Gates to warp to other \n" +
        "planets and avoid being shot by canons, bosses, opponents or touched by\n" +
        "a spider. The spacecraft is cloaked before the first battle. Touch an opponent  \n" +
        "to battle. Marshal can shoot and artefacts give special abilities. \n\n" +
        "In battle the highest rank wins except Spy[1] wins vs Marshal[10] and \n" +
        "Miner[3] wins vs Bomb[B]. Both loose for equal ranks. Flag[F] vs Flag[F]  \n" +
        "is a draw and defeating the opponents Flag[F] wins the game. \n\n" +
        "When both teams have selected a Flag, then the player assigned by be the host \n" +
        "can start the game. Please select a space stratego character from the list \n" +
        "in the left hand side of the screen after choosing a team. Have fun. \n";

    text(rules, centerX, centerY - 230);
}
...
function drawGameSetup() {
    ...
    if (!me.isReady) {
        ...
        textSize(20);
        textAlign(CENTER);
        text("Enter your player name and choose a team:", IMAGE_RING_X + CIRCLE_RADIUS + 100, IMAGE_RING_Y + CIRCLE_RADIUS + 260);
...
function createNameInput() {
    let inputX = IMAGE_RING_X + CIRCLE_RADIUS - 40;
    let inputY = IMAGE_RING_Y + CIRCLE_RADIUS + 325;
...
function setPlayerInfo(team) {
    ...
    if (playerDisplayName.length > 0) {
        ...
        me.isReady = true;
        if (team === 'blue') {
            me.planetIndex = shared.startPlanetTeamBlue;
        } else {
            me.planetIndex = shared.startPlanetTeamGreen;
        }
...
function createStartGameButton() {
    ...
    let buttonX = IMAGE_RING_X + CIRCLE_RADIUS;
    let buttonY = IMAGE_RING_Y + CIRCLE_RADIUS + 325;

    startGameButton = createButton('Start Game');
    // Reposition to the middle of the screen
    let buttonWidth = 180;
    let buttonHeight = 50;
    startGameButton.position(buttonX, buttonY);
...
function drawCharacterList() {
    ...
    myTeamPlayers.forEach(p => takenMap.set(p.characterInstanceId, p.playerDisplayName));
    ...
    characterList.forEach(item => {
        if (!item.isPermanentlyLost) {
            if (takenInstanceIds.has(item.instanceId)) {
                ...
                item.takenByPlayerId = myTeamPlayers.find(p => p.characterInstanceId === item.instanceId)?.playerNumber || null;
            } else {
...
function mousePresse2() {
    if (me.status === 'inBattle' && me.battleOutcome.result !== 'pending') {
        console.log(`Player ${me.playerDisplayName} acknowledged battle outcome: ${me.battleOutcome.result}`);
...
function handleMovement2() {
    ...
    for (const other of activeSpacecrafts) {
        if (other && other.hasCharacter && other.status !== 'lost' && other.playerNumber !== me.playerNumber) {
        ...
        if (myDef && !myDef.isFlag) {
            ...
            for (const guest of activeSpacecrafts) {
                if (guest && guest.hasCharacter && guest.team !== me.team && guest.characterId === 'F' && guest.status !== 'lost') {
                    let d = dist(me.xGlobal + me.xLocal, me.yGlobal + me.yLocal, guest.xGlobal + guest.xLocal, guest.yGlobal + guest.yLocal);
                    if (d < (me.size / 2 + guest.size / 2) * 0.95) {
                        console.log(`Player ${me.playerDisplayName} touching enemy flag! Triggering win.`);
        ...
        for (const guest of activeSpacecrafts) {
            if (guest && guest.hasCharacter && guest.team !== me.team && guest.status !== 'lost') {
                ...
                if (canInitiateBattle) {
                    ...
                    let d = dist(me.xGlobal + me.xLocal, me.yGlobal + me.yLocal, guest.xGlobal + guest.xLocal, guest.yGlobal + guest.yLocal);
                    if (d < (me.size / 2 + guest.size / 2) * 0.95) {
                        console.log(`Collision detected: ${me.playerDisplayName}(${me.characterId}) vs ${guest.playerDisplayName}(${guest.characterId})`);
                        startBattle(guest);
                        return;
                    }
                }
...
function startBattle(opponent) {
    ...
    if (opponent.inBattleWith !== null) {
        console.warn(`Attempted startBattle when opponent ${opponent.playerDisplayName} is already in battle with ${opponent.inBattleWith}. Aborting.`); return;
    }
    if (opponent.status !== 'available' && !opponentIsImmovableTarget) {
        console.warn(`Attempted startBattle when opponent ${opponent.playerDisplayName} status was ${opponent.status} (and not immovable). Aborting.`); return;
    }

    console.log(`Battle started: ${me.playerDisplayName}(${me.characterId}) initiates vs ${opponent.playerDisplayName}(${opponent.characterId})`);
    ...
    if (!myDef || !opDef) {
        ...
        me.battleOutcome = { result: 'loss', opponentInfo: { id: opponent.playerNumber, name: opponent.characterName, rank: opponent.characterRank } };
        me.status = 'inBattle';
        me.inBattleWith = opponent.playerNumber;
        me.isRevealed = true;
        return;
    }
    ...
    console.log(`Battle Result Calculated: ${calculatedOutcome} for ${me.playerDisplayName}`);
...
function handlePlayerLoss() {
    console.log(`Player ${me.playerDisplayName} lost ${me.characterName} (${me.characterInstanceId})`);
    ...
    setTimeout(() => {
        ...
        if (me.status === 'lost' && shared.gameState === 'IN-GAME' && !shared.coreCommandLost) {
            ...
            console.log(`${me.playerDisplayName} now needs a character.`);
        }
    }, 1500);
}
    ...
    if (shared.gameState === "IN-GAME") {
        ...
        activeSpacecrafts.forEach(p => {
            ...
            if (p.status === 'wonGameTrigger') {
                ...
                if ((p.team === 'blue' && greenFlagExists) || (p.team === 'green' && blueFlagExists)) {
                    ...
                    console.warn(`HOST: Invalid win trigger by ${p.playerDisplayName} - opponent flag already captured.`);
        ...
        if (winTriggerPlayer) {
            if (winTriggerPlayer.team === 'blue') { // Already validated greenFlagExists above
                ...
                newWinningTeam = "blue";
                shared.winningPlayerName = winTriggerPlayer.playerDisplayName; // Store winner name
                console.log(`HOST: Win confirmed by ${winTriggerPlayer.playerDisplayName} (Blue).`);
            } else if (winTriggerPlayer.team === 'green') { // Already validated blueFlagExists above
                newGameState = "GAME-FINISHED";
                newWinningTeam = "green";
                shared.winningPlayerName = winTriggerPlayer.playerDisplayName; // Store winner name
                console.log(`HOST: Win confirmed by ${winTriggerPlayer.playerDisplayName} (Green).`);
            }
...
function resetClientState() {
    console.log(`Client Resetting State for ${me.playerDisplayName || 'New Player'}...`);
