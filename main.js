...
/* only out commented for faster performance doing testing
  push()
...
//  minimapImg = Array(totalNumberOfPlanets).fill().map(() => []);
  ...
  fixedMinimap = new BasicMinimap(xSolarSystemCenter = 1250, ySolarSystemCenter = 1000, diameter = 300, color = 'grey', diameterPlanet = screenLayout.diameterPlanet);
...
function generateTowers(count) {
  ...
  const towerTable = [
    ...
    { x: 1000, y: 1200, color: 'blue' },
    { x: 1000, y: 1400, color: 'green' },
    { x: 1000, y: 1600, color: 'yellow' },
    { x: 1200, y: 1000, color: 'purple' },
    { x: 1200, y: 1200, color: 'orange' },
    { x: 1200, y: 1400, color: 'pink' },
    { x: 1200, y: 1600, color: 'brown' },
    { x: 1400, y: 1000, color: 'cyan' },
    { x: 1400, y: 1200, color: 'magenta' },
    { x: 1400, y: 1400, color: 'lime' },
    { x: 1400, y: 1600, color: 'teal' },
    { x: 1600, y: 1000, color: 'lavender' },
    { x: 1600, y: 1200, color: 'maroon' },
    { x: 1600, y: 1400, color: 'olive' },
    { x: 1600, y: 1600, color: 'gray' },
    { x: 1800, y: 1000, color: 'black' },
    { x: 1800, y: 1200, color: 'white' },
    { x: 1800, y: 1400, color: 'gold' },
    { x: 1800, y: 1600, color: 'silver' },
    { x: 2000, y: 1000, color: 'navy' },
    { x: 2000, y: 1200, color: 'maroon' },
    { x: 2000, y: 1400, color: 'olive' },
    { x: 2000, y: 1600, color: 'purple' },
    { x: 2200, y: 1000, color: 'teal' },
    { x: 2200, y: 1200, color: 'lime' },
    { x: 2200, y: 1400, color: 'pink' },
    { x: 2200, y: 1600, color: 'brown' },
    { x: 2400, y: 1000, color: 'cyan' },
    { x: 2400, y: 1200, color: 'magenta' },
    { x: 2400, y: 1400, color: 'gray' },
    { x: 2400, y: 1600, color: 'black' }
  ];
...
function preload() {
  partyConnect("wss://p5js-spaceman-server-29f6636dfb6c.herokuapp.com", "jkv-stategoV7d");
  ...
  shared = partyLoadShared("shared", {
    ...
    showGrafics: false,
    showStarSystem: true,
    startPlanetTeamBlue: 0,
    startPlanetTeamGreen: 1,
  ...
  /* only out commented to make the page load faster doing testing
  for (let i = 0; i < 13; i++) {
...
//  counters++;
  ...
  // Auto player select
  if (firstRun) {
//    console.log("me ", me.playerNumber);

    let selectableCharacters;
    let team;
    if (me.playerNumber === 0) {
      selectableCharacters = characterList.filter(item => item.instanceId === '8_0');
      team = "blue"
    }
    if (me.playerNumber === 1) {
      selectableCharacters = characterList.filter(item => item.instanceId === '4_0');
      team = "green"
    }
    if (me.playerNumber === 2) {
      selectableCharacters = characterList.filter(item => item.instanceId === 'F_0');
      team = "blue"
    }
    if (me.playerNumber === 3) {
      selectableCharacters = characterList.filter(item => item.instanceId === 'F_0');
      team = "green"
    }
    me.team = team;
    me.characterId = selectableCharacters[0].id;
    me.characterRank = selectableCharacters[0].rank;
    me.characterName = selectableCharacters[0].name;
    me.characterInstanceId = selectableCharacters[0].instanceId;
  ...
  // Load the frames for the first time
...
// only out commented for faster performance doing testing
    ...
    case "IN-GAME":
      ...
      //console.log("Before drawInGame");
      drawInGame();
      //console.log("After drawInGame");
...
function removeIfLost(){

  let myOpponents = activeSpacecrafts.filter(p => p.inBattleWith === me.playerNumber);

  if (myOpponents.length > 0) {
    let myOpponent = myOpponents[0];

    if (myOpponent.battleOutcome.result === 'won') {
      fill(0, 255, 0);
      message = `You lost against ${myOpponent.characterName}!`;
    }
  }
}
function showBattleCardsAndMessage(){
  // Draw battle cards and message
  ...
  */
  ...
  const infoX = 200;
  const infoStartY = SCREEN_HEIGHT - 300;
  ...
  text(`TeamNumber: ${me.teamNumber}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`Team: ${me.team}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`characterId: ${me.characterId}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`characterRank: ${me.characterRank}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`characterName: ${me.characterName}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`characterInstanceId: ${me.characterInstanceId}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`isReady: ${me.isReady}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`hasCharacter: ${me.hasCharacter}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`isRevealed: ${me.isRevealed}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`hasBattled: ${me.hasBattled}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`My Status: ${me.status}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`inBattleWigh: ${me.inBattleWith}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`BattleOutcome: ${me.battleOutcome}`, infoX, currentY);
  currentY += infoLineHeight;

  text(`BattleOutcome.opponentInfo: ${me.battleOutcome.opponentInfo}`, infoX, currentY);
  currentY += infoLineHeight;
...
function drawCards(){ 
  // Draw cards for the battle
  const cardWidth = 100;
  const cardHeight = 150;
  const cardSpacing = 10;

  const startX = width / 2 - (cardWidth + cardSpacing) * 2;
  const startY = height / 2 - cardHeight / 2;

  fill(255);
  rect(startX, startY, cardWidth, cardHeight);
  rect(startX + cardWidth + cardSpacing, startY, cardWidth, cardHeight);

  fill(0);
  textSize(16);
  textAlign(CENTER, CENTER);
  text("Your Card", startX + cardWidth / 2, startY + cardHeight / 2);
  console.log("InBattleWith : ", me.inBattleWith);
  console.log("Oppenent : ", me.battleOutcome.opponentInfo);

  text("Opponent's Card", startX + cardWidth * 1.5 + cardSpacing, startY + cardHeight / 2);
}


function drawGameInProgress() {
  //console.log("drawGameInProgress");
  ...
  activeSpacecrafts.forEach((spacecraft) => {
    if (spacecraft.planetIndex >= 0) {
      spacecraft.drawScore(offSetX, offSetY);
      offSetY += 20;
  ...
  fill('gray');
  text("Total number of players: ${activeSpacecrafts.length}", offSetX, offSetY);
  offSetY += 20;
  text("Total number of bullets: " + numberOfBullets, offSetX, offSetY);
  offSetY += 20;
  text("Number of visible bullets: " + numberOfVisualBullets, offSetX, offSetY);
  offSetY += 40;
  text("Performance controls (Only Host):", offSetX, offSetY);
  offSetY += 20;
  text("Key o: show grafics ", offSetX, offSetY);
  offSetY += 20;
  text("Key p: show color blur effect ", offSetX, offSetY);
  offSetY += 20;
  text("Key i: show star system", offSetX, offSetY);
  offSetY += 20;
  if (partyIsHost()) {
    text("Key 9, 8, 7, 6: NoOfCanons 24, 12, 3, 1 ", offSetX, offSetY);
    offSetY += 20;
    text("Key l, k, j: Shooting interval 500, 1000, 2000", offSetX, offSetY);
    offSetY += 20;
  }
...
function keyPressed() {
  ...
  if (keyCode === 80) { // p 
    shared.showBlurEffects = !shared.showBlurEffects;
  }
  if (keyCode === 79) { // o
    shared.showGrafics = !shared.showGrafics;
  }
  if (keyCode === 73) { // i
...
function performHostAction() {
  ...
  if (me.planetIndex === 0) {
    ...
    //    shared.canonTowerCount = canonTowerCount;
    //    shared.canonTowerShootingInterval = canonTowerShootingInterval;
  }
}
...
// Outcommented while developing
//    gameImageManager.checkImageClicks();
    mousePresse2()
  } else if (shared.gameState === "IN-GAME") {
...
function createSpacecrafts() {
  for (let i = 0; i < 13; i++) {
    spacecrafts.push(new Spacecraft({
      ...
      teamNumber: 0,
      xLocal: screenLayout.cropWidth / 2 + 100 * i - 350,
      yLocal: screenLayout.cropHeight / 2,
      xGlobal: screenLayout.diameterPlanet / 2 - screenLayout.cropWidth / 2 + 100 * i,
